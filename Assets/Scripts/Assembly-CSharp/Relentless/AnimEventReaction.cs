...
namespace Relentless
{
	...
	public class AnimEventReaction : GameEventReaction
	{
		...
		public override void React(GameObject gameObject, params object[] paramlist)
		{
			...
			{
			case Operation.Play:
				if (gameObject.animation[text] == null)
				{
					Logging.LogError("Animation clip not found: " + text);
					break;
				}
				if (text.Contains("rinking") || text.Contains("Idle") || text.Contains("idle"))
					UnityEngine.Debug.Log("[AnimER] " + text + " on=" + gameObject.name + " layer=" + gameObject.animation[text].layer + " queued=" + AnimQueued + " playMode=" + AnimPlayMode + " wrapMode=" + AnimWrapMode);
				if (AnimQueued)
				{
