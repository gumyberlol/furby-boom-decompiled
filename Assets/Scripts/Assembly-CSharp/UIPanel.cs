...
public class UIPanel : MonoBehaviour
{
	...
	private void Start()
	{
		...
		// WIDESCREEN PATCH (disabled) -- re-enable to expand panel clip range for widescreen
		//if (mClipping != UIDrawCall.Clipping.None && mClipRange.z > 0f && mClipRange.w > 0f)
		//{
		//	float aspect = (float)Screen.width / (float)Screen.height;
		//	int newWidth = (int)(mClipRange.w * aspect);
		//	if ((float)newWidth != mClipRange.z)
		//	{
		//		clipRange = new Vector4(mClipRange.x, mClipRange.y, newWidth, mClipRange.w);
		//	}
		//}
	}
